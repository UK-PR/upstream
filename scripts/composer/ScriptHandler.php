...
class ScriptHandler {
  ...
  public static function prepareForPantheon() {
    ...
    // Fix up .gitignore: replace with the one for the artifact
    $gitignoreRepoFile = getcwd() . '/.gitignore';
    $gitignoreArtifactFile = getcwd() . '/.gitignore-artifact';

    if ($fs->exists($gitignoreArtifactFile)) {
      $gitignoreContents = file_get_contents($gitignoreArtifactFile);
      file_put_contents($gitignoreRepoFile, $gitignoreContents);
      $fs->remove($gitignoreArtifactFile);
    }
  }
}